...
class DataCleaner:
    ...
    def read_data(self, smm, input_file):
        """
        read input data
        """
        data = []
    ...
    def write2file(self, cleaned_data, num_batches, out_file_name):
        """
        write to file
        """
        with open(out_file_name, "w", encoding="utf8") as fw:
    ...
    def clean(self):
        ...
        out_dir = self.config["basic"].get("output")
        shared_memory_manager.start()
        for input_file in input_files:
            shared_data, num_lines = self.read_data(shared_memory_manager, input_file)
            ...
            tmp = input_file[input_file.rindex("\\")+1:]
            print(tmp)
            input_file_name = tmp[:tmp.rindex(".")]
            print(input_file_name)
            out_path = out_dir + "\\"+ input_file_name + ".jsonl"
            self.write2file(cleaned_data, num_batches, out_path)
